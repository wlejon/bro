...
- **brosurface.** Single-source WebIDL code generation toolchain. Emits the natives of bro's engine-owned namespaces (`src/bronze_host/natives/<sub>/`: prototypes, registrations, JS wrappers), JSDoc documentation (`docs/`), and global TypeScript definitions (`types/index.d.ts` and `docs/bro.d.ts`). A sibling library's namespace is declared there for its docs and typings only; its natives are hand-written in the sibling's `src/api/`. See [brosurface](https://github.com/wlejon/brosurface).
