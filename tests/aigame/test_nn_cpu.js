...
// Tensor basics. nn.createTensor answers an ordinary bro.tensor GpuTensor
// (docs/ai-nn-api.js: the old AITensor with fromArray/toArray/get/set is
// gone), so its element API is upload()/download().
{
    const t = nn.createTensor(3, 2);
    assert(t instanceof bro.tensor.GpuTensor, 'createTensor answers a GpuTensor');
    assert(t.rows === 3 && t.cols === 2 && t.size === 6, 'tensor shape');
    t.upload(new Float32Array([1, 2, 3, 4, 5, 6]));
    assert(t.rows === 3 && t.cols === 2, 'upload of rows*cols elements keeps the shape');
    const arr = t.download();
    assert(arr instanceof Float32Array && arr.length === 6, 'download Float32Array len 6');
...
// PolicyValueNet forward.
{
    ...
    x.upload(new Float32Array(inDim).fill(0.1));
    ...
    allFinite(logits.download(), 'logits');

    // A plain Float32Array is accepted wherever a tensor is, viewed in place.
    const xs = new Float32Array(inDim).fill(0.1);
    const ls = new Float32Array(numActions);
    const vs = net.forward(xs, ls);
    assert(Math.abs(vs - v) < 1e-5, 'Float32Array forward matches tensor forward: ' + vs + ' vs ' + v);
    const lt = logits.download();
    for (let c = 0; c < numActions; c++) {
        assert(Math.abs(ls[c] - lt[c]) < 1e-5, 'Float32Array logits written in place, col ' + c);
    }

    // forwardBatched: B rows in one call must match B independent single-row
    // forward() calls (within float tolerance). Row-major B x inDim.
    const B = 3;
    const xB = nn.createTensor(B, inDim);
    const rowsIn = [];
    const packed = new Float32Array(B * inDim);
    for (let r = 0; r < B; r++) {
        const row = new Float32Array(inDim);
        for (let c = 0; c < inDim; c++) row[c] = 0.1 * (r + 1) + 0.01 * c;
        rowsIn.push(row);
        packed.set(row, r * inDim);
    }
    xB.upload(packed);
    assert(xB.rows === B && xB.cols === inDim, 'batched input keeps B x inDim');
    ...
    for (let r = 0; r < B; r++) {
        ...
        assert(Math.abs(vB[r] - vr) < 1e-3,
            'forwardBatched value row ' + r + ' matches single forward: ' + vB[r] + ' vs ' + vr);
        const l1 = lr.download();
        for (let c = 0; c < numActions; c++) {
            assert(Math.abs(lB[r * numActions + c] - l1[c]) < 1e-3,
...
// FactoredPolicyHead.
{
    ...
    emb.upload(new Float32Array(embed).fill(0.2));
    const logits = nn.createTensor(ph.totalLogits, 1);
    ph.forward(emb, logits);
    allFinite(logits.download(), 'factored logits');
}
...
// DeepSetsEncoder.
{
    ...
    obs.upload(new Float32Array(bro.ai.game.OBS_TOTAL).fill(0.1));
    const embed = nn.createTensor(enc.outDim, 1);
    enc.forward(obs, embed);
    allFinite(embed.download(), 'enc embed');
}
