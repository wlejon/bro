...
# boot bro-headless on appdir_wild under bro-headless's driver, and compare
...
BIN="$(bh_find_bro_headless "$PROJECT_DIR")" || {
    echo "  SKIP  bronze_host_wild  (bro-headless not built)"
    exit 77   # the automake convention for "skipped", not "passed"
}

MODULE="$(bh_ensure_module "$PROJECT_DIR" "$SCRIPT_DIR/appdir_wild"                            "$SCRIPT_DIR/apps/wild_orbit_probe.js")"
case $? in
    0)  ;;
    77) echo "  SKIP  bronze_host_wild  (no bronze CLI in this tree)"
        echo "        Configure with -DBRONZE_WITH_LLVM=ON to build one."
        exit 77 ;;
    *)  echo "  FAIL  bronze_host_wild  (wild_orbit_probe.js did not compile)"
        exit 1 ;;
esac

APP_DIR="$(bh_to_win_path "$SCRIPT_DIR/appdir_wild")"
DRIVER="$(bh_to_win_path "$SCRIPT_DIR/drive_wild.js")"
