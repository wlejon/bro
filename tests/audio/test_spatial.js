...
assert(!threw, 'listener configuration does not throw'); // BUG: listener
...
assert(left.l > left.r, 'source on left -> L > R (L=' + left.l + ' R=' + left.r + ')'); // BUG: spatial-pan-left
assert(right.r > right.l, 'source on right -> R > L (L=' + right.l + ' R=' + right.r + ')'); // BUG: spatial-pan-right
...
assert(nearPeak > farPeak, 'near source louder than far (' + nearPeak + ' vs ' + farPeak + ')'); // BUG: spatial-distance
...
for (const m of ['inverse', 'linear', 'exponential']) {
    ...
    ctx.removeVoice(v);
    assert(!t, 'distance model ' + m + ' accepted'); // BUG: dist-model
}
...
assert(!threw, 'head model setters do not throw'); // BUG: head-model
