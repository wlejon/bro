...
assert(typeof clipId === 'number' && clipId >= 0, 'createClip returns valid id, got ' + clipId); // BUG: createClip

const n = ctx.getClipSampleCount(clipId);
console.log('sample count:', n);
assert(n === N, 'getClipSampleCount matches input, expected ' + N + ' got ' + n); // BUG: clip-sample-count

const ch = ctx.getClipChannels(clipId);
console.log('channels:', ch);
assert(ch === 1, 'getClipChannels is 1 for mono, got ' + ch); // BUG: clip-channels
...
assert(wf && wf.length === bins * 2, 'getClipWaveform returns length ' + (bins*2) + ', got ' + (wf && wf.length)); // BUG: clip-waveform-len
...
assert(minOk, 'all waveform bin mins near -0.5'); // BUG: clip-waveform-min
assert(maxOk, 'all waveform bin maxes near +0.5'); // BUG: clip-waveform-max

// playClip returns a playback id
const pbId = ctx.playClip(clipId, 1.0, false);
console.log('playback id:', pbId);
assert(typeof pbId === 'number' && pbId >= 0, 'playClip returns playback id, got ' + pbId); // BUG: playClip
sleep(50);
const pos = ctx.getPlaybackPosition(pbId);
console.log('position:', pos);
assert(typeof pos === 'number' && pos >= 0, 'getPlaybackPosition returns non-negative number, got ' + pos); // BUG: playback-pos
