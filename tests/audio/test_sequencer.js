...
assert(seq, 'createSequence returns sequence'); // BUG: createSequence

seq.setBPM(120);
console.log('bpm:', seq.bpm);
assert(seq.bpm === 120, 'BPM set/get: expected 120 got ' + seq.bpm); // BUG: seq-bpm

let threw = false;
try { seq.setTimeSignature(4, 4); } catch (e) { threw = true; console.log('setTimeSignature threw:', e.message); }
assert(!threw, 'setTimeSignature does not throw'); // BUG: seq-timesig
...
assert(seq.noteCount === 3, 'noteCount after 3 adds = 3, got ' + seq.noteCount); // BUG: seq-notecount

seq.removeNote(0);
console.log('noteCount after remove:', seq.noteCount);
assert(seq.noteCount === 2, 'noteCount after 1 remove = 2, got ' + seq.noteCount); // BUG: seq-noteremove

seq.clearNotes();
assert(seq.noteCount === 0, 'clearNotes resets count, got ' + seq.noteCount); // BUG: seq-clear

// Loop config
seq.setLoopEnabled(true);
seq.setLoopRange(0, 4);
console.log('loopEnabled:', seq.loopEnabled);
assert(seq.loopEnabled === true, 'loopEnabled is true after setLoopEnabled(true), got ' + seq.loopEnabled); // BUG: seq-loop

// Playing state
assert(seq.playing === false, 'sequence not playing before play(), got ' + seq.playing); // BUG: seq-playing-default
seq.play();
console.log('playing after play():', seq.playing);
assert(seq.playing === true, 'sequence playing after play(), got ' + seq.playing); // BUG: seq-play

seq.stop();
assert(seq.playing === false, 'sequence not playing after stop(), got ' + seq.playing); // BUG: seq-stop

// Automation lanes
let lastVal = -999;
const laneIdx = seq.addAutomationLane((v) => { lastVal = v; });
console.log('lane idx:', laneIdx, 'automationLaneCount:', seq.automationLaneCount);
assert(typeof laneIdx === 'number' && laneIdx >= 0, 'addAutomationLane returns idx, got ' + laneIdx); // BUG: addAutomationLane
assert(seq.automationLaneCount === 1, 'automationLaneCount = 1, got ' + seq.automationLaneCount); // BUG: auto-count
...
assert(!threwInterp, 'all 3 interp modes accepted'); // BUG: auto-interp

seq.clearAutomationPoints(laneIdx);
seq.removeAutomationLane(laneIdx);
console.log('lanes after remove:', seq.automationLaneCount);
assert(seq.automationLaneCount === 0, 'automationLaneCount = 0 after remove, got ' + seq.automationLaneCount); // BUG: auto-remove
