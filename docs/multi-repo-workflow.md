...
(`bronze/.github/actions/checkout-toolchain/action.yml`; inputs `bronze-ref` / `brass-ref`, default `main`.) The action only checks the two trees out. bronze and brass then compile *inside the sibling's build tree*, with the job's own generator, compiler, CRT and compiler launcher, because the api is bound across bronze's C++ embed boundary, which needs the same compiler and CRT on both sides and cannot be checked at load time; a bronze prebuilt elsewhere (bronze's nightly zips, a `BRASS_ROOT` pointed at a separately built brass) would be a silent ABI hazard rather than a shortcut. The sibling's `<name>_test_api` ctest is then the check that the library works the way bro uses it: a bronze realm, the sibling's `install*()`, its namespace driven from JavaScript. brokit had this shape first; brotensor followed on 2026-09-16, and the remaining nine (`broaudio`, `bromesh`, `broflora`, `brogameagent`, `brolm`, `brodiffusion`, `broimage`, `brosoundml`, `brovisionml`) fail at configure with "bronze not found" until they do.

**Apple Silicon.** bronze's code generator emits x86_64 only, so bronze's own CI builds macOS as `CMAKE_OSX_ARCHITECTURES=x86_64`. bro and the siblings build macOS **arm64** (bro ships arm64; the Metal backend is arm64), where the bronze runtime compiles natively but the compiled JS cannot: `src/api/js/*.js` is replaced by a no-op entry (`bronze_js_stubs_arm64.cpp`, `js_entry_stubs.cpp` in bro) and nothing bronze-compiled runs. A sibling must not register its api test there — brotensor exports `BROTENSOR_API_COMPILED_JS` from `src/api/CMakeLists.txt` and skips `brotensor_test_api` on it with a configure-time message — and bronze, as a subproject, must not force the architecture (its `x86_64` default is top-level only since 2026-09-16; before that, an arm64 sibling configure that did not pass `CMAKE_OSX_ARCHITECTURES` flipped every target after `add_subdirectory(bronze)` to x86_64). Until bronze targets arm64, macOS is the one platform where the ecosystem's JavaScript path has no CI at all.
