...
**Apple Silicon.** brass emits machine code for x86_64 only — `src/target/aarch64` is encoder and ABI scaffolding with nothing behind it, and nothing in the pipeline rejects an aarch64 target: the library builds and links on arm64 and the first attempt to *run* what it produced is SIGBUS — so bronze's own CI builds macOS as `CMAKE_OSX_ARCHITECTURES=x86_64`. bro and the siblings build macOS **arm64** (bro ships arm64; the Metal backend is arm64), where the bronze runtime compiles natively but nothing bronze-compiled can run. brass declares this once: **`BRASS_HOST_BACKEND`**, a `CACHE INTERNAL` set in brass's `CMakeLists.txt` from the build's target architecture (ON for x86_64/AMD64), visible from any directory of a build that `add_subdirectory()`s brass however deep, plus a macro of the same name on the brass targets. Everything that executes generated code keys off it and nothing guesses from `APPLE` + `CMAKE_OSX_ARCHITECTURES` any more: the compiled-JS siblings (brokit, broflora, brotensor) and bro swap `src/api/js/*.js` for a no-op entry (`bronze_js_stubs_arm64.cpp`, `js_entry_stubs.cpp` in bro); brotensor builds `brotensor_cpu` without the brass trace JIT (`BROTENSOR_HAS_BRASS_JIT` off, the plain kernels); and every sibling `tests/CMakeLists.txt` builds its api test executable (the check that the embed links) but registers it only when `BRASS_HOST_BACKEND` is ON, with a configure-time `not registered` message — brokit's per-file JS tests and brotensor's `brotensor_test_jit_trace` / `_cpu_jit` / `_cpu_jit_quant` likewise. When the aarch64 emitter lands, the match in brass's `CMakeLists.txt` is the one line to extend. Separately, neither bronze nor brass, as a subproject, may force the architecture (both `x86_64` defaults are top-level only since 2026-09-16; before that, an arm64 sibling configure that did not pass `CMAKE_OSX_ARCHITECTURES` had every target after the `add_subdirectory` built x86_64 and the ones before it arm64, and `libbronze_runtime_shared.dylib` failed to link). Until then, macOS is the one platform where the ecosystem's JavaScript path has no CI at all.
