...
| `natives/<sub>/` | brosurface's output per engine-owned namespace (`gen/emit_natives.mjs` in `../brosurface`): `native_<sub>_decl.h`, the prototypes the compiler checks the bodies against; `native_<sub>_register.cpp`, the `embed::registerNative` calls under `__bro_native.<sub>`; `<sub>.js`, the wrapper; `module.globals`. Every directory here is compiled (its register TU is named in `CMakeLists.txt`) and is an engine-owned namespace; a sibling library's namespace (`bro.mesh`, `bro.lm`, `bro.tensor`, `bro.flora`, `bro.mic`, …) has no directory here, its natives being hand-written in the sibling's `src/api/` |
