...
| `natives/<sub>/` | hand-maintained native declarations per engine-owned namespace: `native_<sub>_decl.h`, the prototypes the compiler checks the bodies against; `native_<sub>_register.cpp`, the `embed::registerNative` calls under `__bro_native.<sub>`. The JS wrapper over them is `js/<sub>.js` (or `js/bro_core.js` for time, window, settings, paths and the panels' `dunder_bro`), the bare identifiers the wrappers read are listed in `js/module.globals`, and the bodies are the hand-written `native_*.cpp` beside this README (`native_<sub>.cpp`, or `native_physics_*.cpp` / `native_scene_*.cpp` for physics and the 3D families). Adding or changing a native means editing the decl, the registration, the wrapper and the body together (and `js/module.globals` if the wrapper reads a new root). Every directory here is compiled (its register TU is named in `CMakeLists.txt`) and is an engine-owned namespace; a sibling library's namespace (`bro.mesh`, `bro.lm`, `bro.tensor`, `bro.flora`, `bro.mic`, …) has no directory here, its natives being hand-written in the sibling's `src/api/` — the same model |
