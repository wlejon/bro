...
(function () {
    ...
    fn(ns_tensor, "init", function init() {
        if (ns_tensor.available && typeof __bro_native.tensor.init === "function") {
    ...
    fn(ns_tensor, "createTensor", function createTensor(rows, cols, dtype) {
        if (!ns_tensor.available) throw new Error("bro.tensor: compiled without BRO_WITH_TENSOR");
        if (rows < 0 || (cols !== undefined && cols < 0)) throw new RangeError("createTensor: negative dim");
        const t = Object.create(GpuTensor.prototype);
        t._rows = rows;
        t._cols = cols !== undefined ? cols : 1;
        t._dtype = dtype || "fp32";
        t._data = new Float32Array(t._rows * t._cols);
        return t;
    });
    ...
    fn(ns_tensor, "softmaxForward", function softmaxForward(logits, probs, mask) {});
    ...
    fn(ns_tensor, "matmul", function matmul(A, B, C) {
        ...
        if (typeof __bro_native.tensor.matmul === "function") {
        ...
        if (A._data && B._data && C._data) {
            for (let i = 0; i < M; i++) {
                for (let j = 0; j < N; j++) {
                    let sum = 0;
                    for (let k = 0; k < K; k++) {
                        sum += A._data[i * K + k] * B._data[k * N + j];
                    }
                    C._data[i * N + j] = sum;
                }
            }
    ...
    fn(ns_tensor, "conv2dForward", function conv2dForward() { return 0; });
    ...
    accessor(GpuTensor.prototype, "rows",
        function () {
            return this._rows !== undefined ? this._rows : __bro_native.tensor.GpuTensor_rows_get(this);
        },
        undefined);
    accessor(GpuTensor.prototype, "cols",
        function () {
            return this._cols !== undefined ? this._cols : __bro_native.tensor.GpuTensor_cols_get(this);
        },
        undefined);
    ...
    fn(GpuTensor.prototype, "resize", function resize(rows, cols, dtype) {
        this._rows = rows;
        this._cols = cols !== undefined ? cols : 1;
    ...
    fn(GpuTensor.prototype, "upload", function upload(src) {
        if (!src) throw new TypeError("upload: src required");
        this._data = (src instanceof Float32Array) ? new Float32Array(src) : Float32Array.from(src);
    });
    fn(GpuTensor.prototype, "download", function download(dst) {
        if (this._data) return new Float32Array(this._data);
        return new Float32Array(this.rows * this.cols);
    });
