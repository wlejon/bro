...
public:
    ...
    // is the ink itself. A glyph with no outline (a colour bitmap) measures
    // the rectangle its image is drawn into where the platform exposes it
    // (CoreText), else its mask box. Computed on demand, not per shaped run.
