...
// SVG paint-server helpers — mirror skia_backend.cpp (this TU has its own
// buildGradColors/kCSSGradInterp statics).
...
static SkTileMode toTileModeR(GradientPaint::Spread s) {
    switch (s) {
...
static sk_sp<SkShader> makeSvgShader(const GradientPaint& gp, std::span<const ColorStop> stops) {
    ...
    SkMatrix lm = SkMatrix::MakeAll(gp.transform[0], gp.transform[2], gp.transform[4],
                                    ...
                                    0, 0, 1);
    SkTileMode tm = toTileModeR(gp.spread);
...
void RasterRenderer::drawSvgPath(std::string_view d, PathFillRule rule,
...
void RasterRenderer::clipSvgPath(std::string_view d, PathFillRule rule) {
    if (!canvas_ || d.empty()) return;
    auto pathOpt = SkParsePath::FromSVGString(std::string(d).c_str());
    if (!pathOpt) return;
    SkPath path = *pathOpt;
