                             ...
                             JSValueConst removedNodes)
{
    ...
    bool isChildList = strcmp(type, "childList") == 0;
    bool isAttributes = strcmp(type, "attributes") == 0;
    bool isCharacterData = strcmp(type, "characterData") == 0;
    ...
    for (int32_t i = 0; i < len; ++i) {
        ...
        for (int32_t t = 0; t < tLen && !matched; ++t) {
            ...
            bool directMatch = (JS_VALUE_GET_PTR(entryTarget) == JS_VALUE_GET_PTR(target));
            ...
            if (directMatch || subtreeMatch) {
                ...
                if (isChildList) {
                    ...
                    if (typeMatch && attributeName) {
                        JSValue filterVal = JS_GetPropertyStr(ctx, options, "attributeFilter");
                        if (JS_IsArray(filterVal)) {
                            typeMatch = false;
                            JSValue fLenVal = JS_GetPropertyStr(ctx, filterVal, "length");
                            int32_t fLen = 0;
                            JS_ToInt32(ctx, &fLen, fLenVal);
                            JS_FreeValue(ctx, fLenVal);
                            for (int32_t f = 0; f < fLen; ++f) {
                                JSValue item = JS_GetPropertyUint32(ctx, filterVal, static_cast<uint32_t>(f));
                                const char* s = JS_ToCString(ctx, item);
                                if (s && strcmp(s, attributeName) == 0) {
                                    typeMatch = true;
                                }
                    ...
                    if (typeMatch) {
                        JSValue aov = JS_GetPropertyStr(ctx, options, "attributeOldValue");
                        includeOldValue = JS_ToBool(ctx, aov);
                    ...
                    if (typeMatch) {
                        JSValue cdov = JS_GetPropertyStr(ctx, options, "characterDataOldValue");
                        includeOldValue = JS_ToBool(ctx, cdov);
                        JS_FreeValue(ctx, cdov);
                    }
                }
        ...
        if (matched) {
            ...
            JS_SetPropertyStr(ctx, record, "oldValue",
                (includeOldValue && oldValue) ? JS_NewString(ctx, oldValue) : JS_NULL);

            JSValue recordsArr = JS_NewArray(ctx);
            JS_SetPropertyUint32(ctx, recordsArr, 0, record);
