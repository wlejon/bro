...
static scene::SkinnedMeshNode* asSkinnedMesh(NodeWrapper* w) {
    if (w && w->node && w->node->type() == scene::SceneNode::Type::Mesh)
...
static JSValue js_node_setSkeleton(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    auto* w = qjsbind::unwrap<NodeWrapper>(ctx, this_val);
...
static JSValue js_node_addClip(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    auto* w = qjsbind::unwrap<NodeWrapper>(ctx, this_val);
    auto* sm = asSkinnedMesh(w);
    if (!sm)
...
static JSValue js_node_getBoneWorldMatrix(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    auto* w = qjsbind::unwrap<NodeWrapper>(ctx, this_val);
    auto* sm = asSkinnedMesh(w);
    if (!sm)
...
static JSValue js_node_stop(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    auto* w = qjsbind::unwrap<NodeWrapper>(ctx, this_val);
    if (!w || !w->node) return JS_UNDEFINED;
    if (w->node->type() == scene::SceneNode::Type::Sprite) {
...
static JSValue js_node_pause(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    auto* w = qjsbind::unwrap<NodeWrapper>(ctx, this_val);
    if (!w || !w->node) return JS_UNDEFINED;
    if (w->node->type() == scene::SceneNode::Type::Sprite) {
        static_cast<scene::SpriteNode*>(w->node)->stop();
...
static JSValue js_node_resume(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    auto* w = qjsbind::unwrap<NodeWrapper>(ctx, this_val);
    if (!w || !w->node) return JS_UNDEFINED;
    if (w->node->type() == scene::SceneNode::Type::Sprite) {
...
static std::function<void()> makeVoidCallback(JSContext* ctx, JSValueConst fnVal) {
    auto ref = std::make_shared<JSFnRef>(ctx, JS_DupValue(ctx, fnVal));
    return [ref]() {
        JSValue fn = JS_DupValue(ref->ctx, ref->fn);
        JSValue r = JS_Call(ref->ctx, fn, JS_UNDEFINED, 0, nullptr);
...
static JSValue js_tween_to(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
...
static JSValue js_tween_parallel(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    if (auto* tw = getTween(ctx, this_val)) tw->parallel();
    return JS_DupValue(ctx, this_val);
}

static JSValue js_tween_call(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    auto* tw = getTween(ctx, this_val);
...
// = forever.
static JSValue js_tween_loop(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    auto* tw = getTween(ctx, this_val);
...
static JSValue js_tween_start(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    auto* tw = getTween(ctx, this_val);
...
static JSValue js_tween_stop(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    if (auto* tw = getTween(ctx, this_val)) tw->stop();
    return JS_DupValue(ctx, this_val);
}

static JSValue js_tween_pause(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    if (auto* tw = getTween(ctx, this_val)) tw->pause();
    return JS_DupValue(ctx, this_val);
}

static JSValue js_tween_resume(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    if (auto* tw = getTween(ctx, this_val)) tw->resume();
    return JS_DupValue(ctx, this_val);
}

static JSValue js_tween_destroy(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
...
static JSValue js_sg_createTween(JSContext* ctx, JSValueConst this_val, int, JSValueConst*) {
    auto* g = getGraph(ctx, this_val);
    if (!g) return JS_UNDEFINED;
