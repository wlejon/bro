...
// The resolution itself lives in src/layout/computed_style.cpp, beside the
// layout boxes and font metrics it reads. It was a static function here until
// a second binding layer needed it: the bronze host (src/bronze_host) hands a
// compiled app the same DOM through a different surface, and two ideas of what
// an element's computed width is would be one too many.
//
// The flush is HERE, on the read, not once when the declaration was built. Per
// CSSOM the object getComputedStyle answers with is LIVE — it keeps tracking
// the element — so a UI that holds one, changes a class or an inline property,
// and reads again must see the second answer. Flushing only at construction
// made every such read report the state the element was in when the object was
// created, for as long as the object was held. Repeated reads are still one
// layout pass: Document::layoutIsCurrent() short-circuits the run.
static std::string getComputedProperty(JSContext* ctx, bro::dom::Element* el,
                                       const std::string& prop) {
    auto* eng = getEngineFromCtx(ctx);
    if (eng && el) eng->flushLayoutForRead(el->document());
    return bro::layout::computedProperty(el, prop,
                                         eng ? eng->textMetrics() : nullptr);
}
