...
void dispatchControlEvent(const ControlContext&, dom::Element* el,
                          dom::Event& event) {
    if (!el) return;
    dom::dispatchDomEvent(el, event);
}
    ...
    if (newInput) {
        ...
        if (itype == layout::ElInput::InputType::Checkbox) {
            ...
            *ctx.dirtyFlag = true;
            ...
            state.activationPrevRadio.assign(ctx.document,
                                             layout::checkedRadioInGroup(target));
            layout::clearRadioGroup(target);
            target->setAttribute("checked", "");
            dom::Event changeEvt("change");
            dispatchControlEvent(ctx, target, changeEvt);
            dispatchInputEvent(ctx, target);
            ...
            *ctx.dirtyFlag = true;
            ...
            if (ctx.overlays) {
                ...
                auto onInput = [dirtyFlag, elem](const std::string& hex) {
                    elem->setAttribute("value", hex);
                    dom::InputEvent evt("input");
                    evt.setData(hex);
                    evt.setIsTrusted(true);
                    dom::dispatchDomEvent(elem, evt);
                    if (dirtyFlag) *dirtyFlag = true;
                };
                auto onCommit = [dirtyFlag, elem](const std::string& hex) {
                    elem->setAttribute("value", hex);
        ...
        *ctx.dirtyFlag = true;
        ...
        if (ctx.overlays) {
            ...
            auto onSelect = [dirtyFlag, elem, optsCopy, sel](int index) {
...
void dispatchDocMouseRelease(
    ...
    if (clickTarget && !isInDisabledControl(clickTarget)) {
        ...
        dom::dispatchDomEvent(target, clickEvt);
        ...
        if (!clickEvt.defaultPrevented() && target) {
            ...
            if (isButton || isActionInput) {
                ...
                if (btnType == "submit") {
                    ...
                    if (owner) {
                        dom::Event submitEvt("submit", true, true);
                        submitEvt.setIsTrusted(true);
                        dom::dispatchDomEvent(owner, submitEvt);
                    }
