...
function buildAgent() {
    ...
    const sim = SwSim.create({
        ...
        timeLimit: 20, stallDecisions: 30,
    });
    const agent = SwAgent.create({ sim });
    return { sim, agent, baseSpawnY: spawn.y };
}
...
const ACTION_MASK = new Float32Array(sim.numActions);
for (let i = 0; i < ACTION_MASK.length; i++) ACTION_MASK[i] = 1;
