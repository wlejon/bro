...
Lightweight app runtime: HTML/CSS/JS apps, GPU-accelerated. ~125K LOC C++20 (`src/js/` bindings are over half). Stack: QuickJS + qjsbind + brokit + htmlayout + broaudio + bromesh + Jolt + Skia (Ganesh-GL on GPU, CPU raster otherwise) + SDL3. All GPU work is OpenGL 3.3 core via glad — no SDL_GPU/D3D12/Metal path.

Two executables, one `Engine` (via `EngineConfig`): `bro` (windowed) and `bro-headless` (JS-scripted; GPU by default through a hidden SDL window — same pipeline incl. WebGL). Headless globals: `screenshot()`, `advanceTime(ms)` (virtual time → deterministic tests), `flush()`, `sleep()`, `assert()`; all standard DOM APIs work. Full reference: [docs/headless.md](docs/headless.md).
